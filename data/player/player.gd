...
@onready var parts: Dictionary = {
	...
	"body": $body,
	"collision": $collision,
	"crosshair": $hud_camera/crosshair
}
...
func _ready() -> void:
	...
	Signals.levelFinished.connect(onLevelFinished)
	timer.timeout.connect(resetScene)
...
func setCrosshairEnabled(enabled: bool):
	parts["crosshair"].visible = enabled
